...
static int check_vnum (lua_State *L)
  ...
  if ( vnum < 1 )
    luaL_error (L, "Vnum %d is less than 1.", vnum);
...
static int L_system_info (lua_State *L)
{
  ...
  INFO_ITEM (CLAN_LIST);
  INFO_ITEM (SHIP_LIST);
  INFO_ITEM (PLANET_LIST);
  INFO_ITEM (SPACE_LIST);
  INFO_ITEM (BOUNTY_LIST);
  INFO_ITEM (DISINTIGRATION_LIST);
  INFO_ITEM (SENATE_LIST);
  INFO_ITEM (GUILD_LIST);
  INFO_ITEM (GUARD_LIST);
...
int loc;
  ...
  for (loc = 0; loc < MAX_WEAR; loc++)
    if (strcasecmp (sloc, w_flags [loc]) == 0)
       break;
...
static int L_gain_exp (lua_State *L)
{
  ...
  gain_exp (L_getchar (L), luaL_checknumber (L, 1), ability);
  return 0;
}  /* end of L_gain_exp */
